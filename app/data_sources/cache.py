...
DEFAULT_TTL = {
    ...
    "kline:30m":        300,     # 内存
    "kline:1H":         300,     # 内存
    "kline:1D":         14400,   # 磁盘 4小时
    "kline:1W":         86400,   # 磁盘 24小时
    "kline:1M":         86400,   # 磁盘 24小时
...
def make_key(data_type: str, *parts) -> str:
    ...
    Examples:
        make_key("kline", "SH600519", "1D", 300)
        → "kline:SH600519:1D:300"
    """
...
class MemoryCache:
    ...
    适用: 行情、分钟线、市场快照等短TTL数据。
    用 OrderedDict 实现 O(1) 淘汰。
    """
...
class DiskCache:
    """
    磁盘缓存 — feather 文件，进程重启不丢。

    目录结构:
      cache/{data_type}/{MARKET_CATEGORY}/{MARKET}{symbol}_{timeframe}.feather
      cache/{data_type}/{MARKET_CATEGORY}/{MARKET}{symbol}.feather

      MARKET_CATEGORY: CN(SH/SZ/BJ), HK, OTHER
      例: cache/kline/CN/SH600519_1D.feather
          cache/kline/HK/HK00700_1D.feather
          cache/stock_info/CN/SZ000001.feather

    每个 symbol 一个文件，崩溃最多丢一只，重取就行。
    """

    def __init__(self, base_dir: str = "cache", max_age_check_interval: int = 300):
        self._base_dir = Path(base_dir)
        self._base_dir.mkdir(parents=True, exist_ok=True)
        self._lock = threading.Lock()  # 写文件时锁
        self._hits = 0
        self._misses = 0
    ...
    def _file_path(self, data_type: str, market: str, symbol: str,
        ...
        Examples:
            _file_path("kline", "SH", "600519", "1D")
            → cache/kline/CN/SH600519_1D.feather

            _file_path("kline", "HK", "00700", "1D")
            → cache/kline/HK/HK00700_1D.feather
        """
        ...
        if timeframe:
            filename = f"{market}{symbol}_{timeframe}.feather"
        else:
            filename = f"{market}{symbol}.feather"
...
class TieredCache:
    ...
    def get(self, key: str, data_type: str) -> Optional[Any]:
        ...
        if should_use_disk(data_type):
            parts = key.split(":")
            # parts[0]=data_type, parts[1]=symbol, parts[2]=timeframe, parts[3]=limit
            if len(parts) >= 2:
                symbol = parts[1]
                if symbol:
...
def cached(data_type: str, ttl: float = None, key_fn: Callable = None):
    """
    缓存装饰器 — 自动拦截重复请求，自动路由到内存/磁盘。

    用法:
        @cached("kline", key_fn=lambda sym, tf, lim: f"{sym}:{tf}:{lim}")
        def get_kline(symbol, timeframe, limit):
            ...
    """
    def decorator(fn):
